...
class View extends Component {
  ...
  searchQuery(event) {
    ...
    axios({
      method: 'get',
      url: `http://localhost:4000/q=${query}`,
      headers: { 'X-Requested-With': 'XMLHttpRequest' },
    })
      .then(response => response.data)
       .then(data => {
          this.setState({...this.state, queryResultsdata});
       })
  }
  ...
  onDrop(files) {
    ...
    axios.post('/pdf/upload', data, config).then((response) => {
  ...
  renderSidebar() {
    ...
    return (
      ...
      >
        ...
        >
          <SearchBox value={this.state.search} searchFunc={this.searchQuery} />
          <UploadFile />
